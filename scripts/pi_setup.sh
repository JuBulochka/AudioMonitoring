...
# Скрипт запускается на Raspberry Pi и собирает параметры, которые нельзя заранее
# зашить в репозиторий: адрес сервера, ключ устройства и пользователя Linux.
echo ""
echo "============================================================"
...
# Reverse-туннель держится на отдельном SSH-ключе, чтобы не использовать пароль Pi.
...
# Публичный ключ платформы добавляется в authorized_keys, чтобы веб-терминал мог
# подключаться к Pi через уже поднятый reverse-туннель.
...
# systemd держит edge-клиент постоянно запущенным и поднимает его после перезагрузки.
